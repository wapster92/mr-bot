...
```
  ...
  Чтобы добавить пользователя как автора без участия в очереди ревьюеров:

```text
/allow @telegramUsername gitlab.username --no-review Имя Фамилия
```
...
- Команда `/users` или кнопка `Пользователи` открывает лидерскую панель. В ней можно:
  ...
  - включить или отключить участие разработчика в очереди ревьюеров;
...
- `isAllowed: false` запрещает доступ, `isActive: false` отключает пользователя.
  ...
  отправляются.
- `isReviewer: false` исключает разработчика из автоматических назначений, команды
  `/review` и напоминаний о ревью, но сохраняет доступ к боту, уведомления по его MR и
  авторский XP. Его approve не учитывается ботом как обязательный approve разработчика.
  Для существующих пользователей и новых записей без этого поля по умолчанию
  используется `isReviewer: true`.
- Комментарии ревьюеров отправляются автору MR и лидам. Если автор ещё не добавлен в
  бот и ему нельзя доставить сообщение, лиды всё равно получают уведомление.
